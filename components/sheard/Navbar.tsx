...
export function Navbar() {
  ...
  const navLinks = [
    ...
    { name: "Why Partner?", href: "/why-join" },
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <div>
                ...
                <p className="text-[16px] font-normal text-[#8a6a3d]">FRESH</p>
              </div>
                ...
                <li key={link.name}>
                  <Link
                    ...
                    className={`text-base font-medium transition-colors relative ${
                      isActive(link.href)
                        ? "text-[#8a6a3d] font-semibold"
                        : "text-[#272727] hover:text-[#8a6a3d] hover:font-semibold"
                    }`}
                  >
                    {link.name}
                  </Link>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-6">
              {isLoggedIn && userRole === "user" && (
                <>
                  <Link href="/messages" className="flex" aria-label="Messages">
                    <MessageCircle className="h-6 w-6 hover:text-[#8a6a3d]" />
                  </Link>
                  <Link
                    href="/cart"
                  ...
                  >
                    <ShoppingCart className="h-6 w-6 hover:text-[#8a6a3d]" />
                    {isCartLoading ? (
                      <span className="absolute -top-2 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-[#8a6a3d] text-xs text-white">
                        ...
                      </span>
              ...
              {isLoggedIn ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="rounded-full focus:outline-none focus:ring-0 focus:ring-offset-0 cursor-pointer">
                      <CircleUser className="h-8 w-8 text-[#8a6a3d]" />
                    </button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-48">
                    <DropdownMenuItem asChild>
                      <Link
                        href={getProfileLink()}
                        className="w-full cursor-pointer hover:text-[#8a6a3d]"
                      >
                        <User className="mr-2 h-4 w-4 text-[#8a6a3d]" />
                    ...
                    {userRole === "user" && (
                      <DropdownMenuItem asChild>
                        <Link
                          ...
                          className="w-full cursor-pointer hover:text-[#8a6a3d]"
                        >
                          <ShoppingBag className="mr-2 h-4 w-4 text-[#8a6a3d]" />
                          Order history
                        </Link>
                      </DropdownMenuItem>
                    )}
                ...
                <Link href="/sign-up">
                  <Button
                    variant="default"
                    className="bg-[#5f4729] hover:bg-[#8a6a3d] text-white cursor-pointer"
                  >
                    Sign Up / Sign In
                  </Button>
                </Link>
              ...
              <Button
                variant="outline"
                size="sm"
                className="h-[40px] text-white bg-[#8a6a3d] hover:bg-[#755933] hover:text-white cursor-pointer whitespace-nowrap border-[#8a6a3d]"
            ...
            <div className="flex items-center gap-2 lg:hidden">
              <Button
                variant="ghost"
                size="icon"
                className="h-[35px] w-auto px-2 text-white bg-[#8a6a3d] hover:bg-[#755933] hover:text-white cursor-pointer whitespace-nowrap"
                onClick={handleDonateClick}
              ...
              <div className="border-t pt-4">
                {isLoggedIn ? (
                  <div className="space-y-4">
                    {userRole === "user" && (
                      <div className="flex gap-4 mb-4">
                        <Link
                          href="/messages"
                          className="flex items-center gap-2 text-[#272727] hover:text-[#8a6a3d]"
                          onClick={() => setIsMenuOpen(false)}
                        >
                    ...
                    <Link
                      href={getProfileLink()}
                      className="flex items-center gap-2 text-[#272727] hover:text-[#8a6a3d] py-2"
                      onClick={() => setIsMenuOpen(false)}
                    >
                  ...
                  <Link href="/sign-up" onClick={() => setIsMenuOpen(false)}>
                    <Button
                      variant="default"
                      className="w-full bg-[#5f4729] hover:bg-[#8a6a3d] text-white mb-4"
                    >
                      Sign Up
                    </Button>
                  </Link>
      ...
      <Dialog open={isDonationModalOpen} onOpenChange={setIsDonationModalOpen}>
        <DialogContent className="w-[90%] max-w-md rounded-xl p-6 sm:p-8">
          ...
          <div className="mt-4 text-justify text-[18px] text-[#595959] leading-[150%]">
            <p className="mb-4">
              Thank you for being part of Table Fresh. Your contribution helps
              us keep the platform accessible, strengthen local food networks,
              and make it easier for people to discover quality produce from
              real growers.
            </p>
            <p className="text-[#595959] mt-7">
              With appreciation <br /> from the <br /> Table Fresh team
            </p>
          </div>
          <div className="mt-6 flex flex-col items-center gap-4">
            ...
            <Button
              ...
              className="w-full rounded-md bg-[#8a6a3d] py-3 text-lg font-semibold text-white hover:bg-[#755933]"
            >
              Continue Support
            </Button>
          </div>
